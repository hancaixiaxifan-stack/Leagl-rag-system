#!/usr/bin/env python3
"""
2×2 Factorial Ablation — Aviation Law Art. 128 (Hard-GT Only)

Ground Truth = Graph BFS topological citations (8 unique articles).
No Soft-GT — avoids the R_soft=0 problem.

Purpose: Cross-domain generalization validation + Group A baseline data.

Groups:
  A: All aviation law articles (no graph, no filter)       — baseline
  B: All aviation law → domain keyword filter              — semantic only
  C: Graph BFS 8 candidates (no filter)                    — topological only
  D: Graph BFS → keyword filter                            — full pipeline

Usage:
    python scripts/ablation_aviation.py
"""
...
TARGET_LAW = "中华人民共和国民用航空法"
TARGET_ARTICLE = "第一百二十八条"
DIRECTION_DESC = "国内航空运输承运人赔偿责任限额调整 (liability_limit_adjustment)"

OLD_TEXT = (
    "第一百二十八条 国内航空运输承运人的赔偿责任限额由国务院民用航空主管部门制定，"
    "报国务院批准后公布执行。"
    "旅客或者托运人在交运托运行李或者货物时，特别声明在目的地点交付时的利益，"
    "并在必要时支付附加费的，除承运人证明旅客或者托运人声明的金额高于托运行李或者货物"
    "在目的地点交付时的实际利益外，承运人应当在声明金额范围内承担责任；"
    "本法第一百二十九条的其他规定，除赔偿责任限额外，适用于国内航空运输。"
)

# Hard-GT: Graph BFS topological citations (deduplicated)
GRAPH_GT = {
    "第一百一十一条",
    "第一百一十二条",
    "第一百一十六条",
...
RUNS_PER_GROUP = 3
OUTPUT_CSV = Path("data/ablation_aviation_comparison.csv")
OUTPUT_JSON = Path("data/ablation_aviation_results.json")
...
# Aviation domain keywords for Art.128
AVIATION_KEYWORDS = {
    "承运人", "赔偿责任", "赔偿限额", "责任限额",
    "旅客", "托运人", "行李", "货物",
    "国内航空运输", "国际航空运输",
...
# ────────────────────────────────────────────────────────────
# Aviation domain keyword filtering
# ────────────────────────────────────────────────────────────


def filter_by_aviation_keywords(
    ...
    for cand in candidates:
        ...
        entry = {**cand, "_aviation_keywords": matched}
        if matched:
            passed.append(entry)
        else:
...
def main():
    print("2×2 Factorial Ablation — Aviation Law Art. 128 (Hard-GT Only)")
    print("=" * 60)
    print(f"Target: {TARGET_LAW} {TARGET_ARTICLE}")
    print(f"Direction: {DIRECTION_DESC}")
    print(f"Ground Truth (Hard-GT): {len(GROUND_TRUTH)} articles — {sorted(GROUND_TRUTH)}")
    print(f"Aviation keywords: {len(AVIATION_KEYWORDS)}")
    ...
    print(f"\n{TARGET_LAW}: {len(all_articles)} active articles (excluding Art.128)")
    ...
    # Aviation keyword filter on all articles
    passed_kw, filtered_kw = filter_by_aviation_keywords(all_articles, AVIATION_KEYWORDS)
    keyword_candidates = sorted(passed_kw, key=art_sort_key)
    print(f"\nAviation keyword filter: {len(keyword_candidates)} passed / {len(all_articles)} total")
    print(f"  Filtered out: {len(filtered_kw)} articles")
    ...
    # D: graph candidates that also match aviation keywords
    passed_d, _ = filter_by_aviation_keywords(graph_candidates, AVIATION_KEYWORDS)
    ...
    # Run 4 groups
    results = {
        "experiment": "factorial_ablation_aviation_hard_gt",
        ...
        "aviation_keywords": sorted(AVIATION_KEYWORDS),
        "groups": {},
    }
    ...
    # Cross-domain note
    print(f"\n{'='*75}")
    print("Cross-Domain Data (for Table)")
    print(f"{'='*75}")
    a_tokens = csv_rows[0]["avg_prompt_tokens"]
    c_tokens = csv_rows[2]["avg_prompt_tokens"]
    print(f"  Domain: Aviation Law (航空法)")
    print(f"  Group A: {csv_rows[0]['candidate_count']} articles, {a_tokens} tokens, "
          f"P={csv_rows[0]['avg_precision']:.4f}")
    print(f"  Group C/D: {csv_rows[2]['candidate_count']} articles, {c_tokens} tokens, "
          f"R_hard={csv_rows[2]['avg_recall']:.4f}")
    print(f"  Token reduction: {(1 - c_tokens/a_tokens)*100:.1f}%")
